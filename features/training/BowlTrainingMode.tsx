...
export function BowlTrainingMode({ 
  ...
  if (gameState === "RESULT_SUCCESS") {
    return (
      <div className="flex flex-col items-center justify-center p-4 min-h-[80vh] w-full max-w-lg mx-auto animate-fade-in text-center">
        <div className="bg-white border-4 border-brand-charcoal rounded-modal p-8 md:p-10 shadow-elevated w-full">
  ...
  if (gameState === "RESULT_FAIL") {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-4 min-h-[80vh] w-full max-w-xl mx-auto animate-fade-in">
          ...
          <div className="bg-brand-linen/40 border-2 border-brand-charcoal rounded-button p-4 md:p-6 mb-8 text-left space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {missing.length > 0 ? (
                  missing.map((item, i) => (
                    <span key={i} className="text-xs font-body font-bold px-2.5 py-1 bg-brand-olives/20 text-brand-olives border-2 border-brand-charcoal rounded-full animate-pulse-fast">
                      {item}
                    </span>
                  ))
                ) : (
  ...
  return (
    <div className="w-full h-full max-w-3xl mx-auto flex flex-col overflow-y-auto px-4 py-3 custom-scroll md:max-h-[92vh]">
      {/* Shell layout */}
      <div className="bg-white border-4 border-brand-charcoal rounded-modal shadow-elevated flex flex-col overflow-hidden animate-slide-up">
        ...
        <div className="p-4 border-b-4 border-brand-charcoal bg-brand-linen/30 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          ...
          <div className="flex items-center gap-3 justify-between sm:justify-end">
            ...
            {timer <= 5 && (
              <div className={`flex items-center gap-1.5 px-3 py-1.5 bg-brand-tomato/10 border-2 border-brand-tomato text-brand-tomato rounded-button text-xs font-bold ${!isReducedMotion ? 'animate-pulse' : ''}`}>
                <IconClock size={14} className="text-brand-tomato animate-bounce" />
